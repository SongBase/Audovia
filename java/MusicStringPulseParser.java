 ...
 */
public class MusicStringPulseParser extends MusicStringParser  // modified by D G Gray 07-jun-2010
{
     ...
     */
    public MusicStringPulseParser()  // modified by D G Gray 07-jun-2010
    {
    ...
    private void parseToken(String s) throws JFugueException
    {
        ...
        switch(s.charAt(0))
        {
            ...
            case '{' : parseStringConstant(s);         break;  //  added D G Gray 03-jul-2014
...
//            throw new JFugueException(JFugueException.VOICE_EXC,voiceNumberString,s);
    ...
    private void parseControllerElement(String s) throws JFugueException
    {
        ...
        if (-1 != controlIndexInt)
        {
            ...
            //if (16383 == controlValue) {  // modified by D G Gray 26/07/2010
    ...
    private void parseDictionaryElement(String s) throws JFugueException
    {
        ...
        //definition.replace('~', ' ');
        definition = definition.replace('~', ' '); // modified D G Gray 03-jul-2014
    ...
    class NoteContext
    {
        ...
        boolean isSharp                 = false;  // added D G Gray 18-jul-2012
        boolean isFlat                  = false;  // added D G Gray 18-jul-2012
    ...
    private void parseNoteElement(String s) throws JFugueException
    {
        ...
        while (context.existAnotherNote) {
        	...
        	context.isStartOfTie = false;  // added by D G Gray 27-feb-2012
        	context.isEndOfTie = false;  // added by D G Gray 27-feb-2012
        	context.isSharp = false;  // added by D G Gray 18-jul-2012
        	context.isFlat = false;  // added by D G Gray 18-jul-2012
        	context.isNatural = false;  // added by D G Gray 18-jul-2012
    ...
    private void parseStringConstant(String s)  //  added D G Gray 03-jul-2014
    {
		 String word = s.substring(1, s.length()-1);
		 String definition = dictionaryLookup(word);
		 Pattern pattern = new Pattern(definition);
		 parseString(pattern);
	 }

    public void parseString(Pattern pattern) throws JFugueException // added D G Gray 26th Aug 2014
    {
        String[] tokens = pattern.getTokens();
    ...
    /** Returns the index with which to start parsing the next part of the string, once this method is done with its part */
     private int parseLetterNote(String s, int slen, int index, NoteContext context)
     {
         ...
         while (checkForModifiers) {
             ...
             {
                 switch(s.charAt(index)) {
                     case '#' : index++; context.isSharp = true; context.noteNumber++;  /*if (context.noteNumber == 12) context.noteNumber = 0; */ break;  // modified D G Gray 18-jul-2012
                     case 'B' : index++; context.isFlat = true; context.noteNumber--;  /*if (context.noteNumber == -1) context.noteNumber = 11;*/ break;  // modified D G Gray 18-jul-2012
    ...
    private void computeNoteValue(NoteContext context)
    {
        ...
        // Adjust for Key Signature // modified D G Gray 18-jul-2012
        if ((keySig != 0) && (!context.isNatural) && (!context.isSharp) && (!context.isFlat)) {
        ...
        if (index < slen) {
            switch (s.charAt(index)) {
                case '/' : index = parseNumericDuration(s, slen, index, context);
                           context.decimalDuration = context.decimalDuration/128.0; // added by D G Gray 26-mar-2011
                           break;
                case 'W' :
                case 'H' :
                case 'Q' :
...
//        context.duration = (long)(ppw * context.decimalDuration) / 4000;
    ...
    {
        MusicStringPulseParser parser = new MusicStringPulseParser();  // modified by D G Gray 07-jun-2101
