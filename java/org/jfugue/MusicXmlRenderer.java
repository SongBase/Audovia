...
package org.jfugue;  // modified by D G Gray 14/6/2010
//import org.jfugue.*;  // added by D G Gray 14/6/2010
...
{	private Element	root;			//	top-level node of entire MusicXML Document
	...
	//private static final int MUSICXMLDIVISIONS = 4;	//	4 divisions per quarter note  // modified by D G Gray 15/06/2010
	private static final int MUSICXMLDIVISIONS = 32;	//	32 divisions per quarter note  // added by D G Gray 15/06/2010
	private static final double WHOLE = 1024.0;
	private static final double QUARTER = 256.0;

    public MusicXmlRenderer(String aSongName)  // modified D G Gray 17/6/2011
    {	root = new Element("score-partwise");
      Element elMovTit = new Element("movement-title"); // added D G Gray 17/6/2011
    	...
    	root.appendChild(elMovTit); // added D G Gray 17/6/2011
    	elMovTit.appendChild(aSongName); // added D G Gray 17/6/2011
    ...
    public void doFirstMeasure(boolean bAddDefaults)
    {
    	...
    	{	elCurMeasure = new Element("measure");
    		...
    		{	//	divisions - 4 per beat
	    		...
	    		//put keysig here
	    		doKeySig(new KeySignature((byte)0, (byte)0));  // added D G Gray 18/06/2010
	    		...
	    		//elBeats.appendChild(Integer.toString(4));  //  modified by D G Gray 19/06/2010
	    		elBeats.appendChild(beats);  //  added by D G Gray 19/06/2010
	    		elTime.appendChild(elBeats);
	    		Element elBeatType = new Element("beat-type");
	    		//elBeatType.appendChild(Integer.toString(4));  //  modified by D G Gray 19/06/2010
    		...
    		if (elAttributes.getChildCount() > 0)
       		...
       		//	key signature
    		//if (bAddDefaults)
    		//	doKeySig(new KeySignature((byte)0, (byte)0));	//	C major  //modified by D G Gray 18/06/2010
    		//if (bAddDefaults)
    		//	doTempo(new Tempo(120));	//	120 BMP default  // modified by D G Gray 18/06/2010
     	}
    }	//	doFirstMeasure
    ...
    {	Element elDirection = new Element("direction");
		...
		//Integer iBPM = new Float(PPMtoBPM(tempo.getTempo())).intValue();  // modified by D G Gray 14/6/2010
		Integer iBPM = new Float((tempo.getTempo())).intValue();  // added by D G Gray 14/6/2010
...
//    	        pattern.add(layer.getMusicString());
 ...
 //       pattern.add(time.getMusicString());
    ...
    {	Element elKey = new Element("key");
    	...
    	Element oldKey = elAttributes.getFirstChildElement("key");  // added by D G Gray 20/06/2010
    	if (oldKey == null) elAttributes.appendChild(elKey);  // added by D G Gray 20/06/2010
    	else elAttributes.replaceChild(oldKey, elKey);  // added by D G Gray 20/06/2010

    	//elAttributes.appendChild(elKey);  // modified by D G Gray 20/06/2010
    	if (bNewAttributes == true)
    		elCurMeasure.appendChild(elAttributes);
    }	//	doKeySig
...
//    	        pattern.add(pitchBend.getMusicString());
    ...
    private void doNote(Note note, boolean bChord)
    {
    	...
    	{	//	pitch
    		...
    		//elOctave.appendChild(Integer.toString(note.getValue() / 12));  // modified by D G Gray 03/07/2010
    		// D G Gray  Reduce octave number by 1 to comply with ISO standard used by most music publishing systems.
    		elOctave.appendChild(Integer.toString((note.getValue() / 12) - 1));  // added by D G Gray 03/07/2010
...
//		Entertainer();
	...
	{	File fileXML = new File("C:\\Documents and Settings\\Philip Sobolik\\My Documents\\"
	    ...
	    try {
	    	...
	    	MusicXmlRenderer MusicXmlOut = new MusicXmlRenderer("FrereJacques");  // modified D G Gray 17/6/2011
...
*/
	...
	{	File fileSrc = new File("F:\\WIN\\JFugue\\org\\jfugue\\extras\\"
	    ...
	    try {
			...
			//	set up the source MusicXML file (parser)
	    	MusicXmlRenderer MusicXmlOut = new MusicXmlRenderer("Entertainer");  // modified D G Gray 17/6/2011
//		    MusicXmlParser.setTracing(Parser.TRACING_ON);
